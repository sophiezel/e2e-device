...
setup_shared() {
  ...
  # 生成 wdio.conf.ts
  cat > "$SHARED/wdio.conf.ts" <<'WDIOCONF'
...
// Resolve specs from sandbox (generated specs)
...
// Appium binary resolution
function resolveAppiumCommand(): string {
  const envBin = process.env.E2E_APPIUM_BIN;
  if (envBin && fs.existsSync(envBin)) return envBin;
  const projectAppium = path.join(projectRoot, "node_modules", ".bin", "appium");
  if (fs.existsSync(projectAppium)) return projectAppium;
  const skillAppium = path.join(skillRoot, "node_modules", ".bin", "appium");
  if (fs.existsSync(skillAppium)) return skillAppium;
  return "appium";
}

// Capabilities — load from skill
let getCapabilities: () => WebdriverIO.Capabilities;
try {
  const appConfig = require(path.join(skillRoot, "config", "app"));
  getCapabilities = appConfig.getCapabilities;
} catch {
  getCapabilities = () => ({}) as WebdriverIO.Capabilities;
}

function applyAndroidSdkEnv(): void {
  // FIXME: load from skill
}
...
export const config: Options.Testrunner = {
  ...
  logLevel: (process.env.E2E_DEBUG === "1" ? "info" : "warn") as Options.WebDriverLogTypes,
  bail: 0,
  waitforTimeout: 30000,
  connectionRetryTimeout: 120000,
  connectionRetryCount: 2,
  port: appiumPort,
  services: process.env.E2E_APPIUM_SKIP_SERVICE === "1" ? [] : [
    ["appium", {
      command: appiumCmd,
      args: {
        port: appiumPort,
  ...
  afterTest: async function() {
    ...
    if (process.env.E2E_SEQUENTIAL_INDIVIDUAL !== "1" && testCount % SESSION_RESET_INTERVAL === 0) {
      console.log(`[wdio] Session reset after ${testCount} tests`);
      try {
