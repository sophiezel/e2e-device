<!-- 触发条件: 新仓库接入 / scaffold 失败 / ensure-host-deps 失败时加载 -->
# 宿主项目接入（通用）
...
`orch_cli` 使用 `$E2E_DEVICE_SKILL_ROOT/scripts/node_modules/.bin/ts-node` 执行宿主目录下的 `e2e-device/orchestration/cli.ts`（cwd 仍为宿主根目录）。
