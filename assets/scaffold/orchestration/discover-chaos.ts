...
export function discoverChaos(domain?: string): CaseEntry[] {
	...
	const sb = sandboxDir();
	const chaosDir = path.join(sb, "chaos");
	...
	for (const scenario of CHAOS_SCENARIOS) {
		cases.push({
			...
			spec: path.join(sb, "chaos", `${d}.${scenario.id}.chaos.spec.ts`),
	...
	for (const scenario of HYBRID_CHAOS_SCENARIOS) {
		cases.push({
			...
			spec: path.join(sb, "chaos", `${d}.hybrid.${scenario.id}.chaos.spec.ts`),
	...
	for (const r of routes) {
		cases.push({
			...
			spec: path.join(sb, "chaos", `${d}.${r.name}.chaos.spec.ts`),
