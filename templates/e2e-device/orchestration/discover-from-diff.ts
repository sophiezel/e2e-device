...
export function discoverFromDiff(_base = "origin/main"): CaseEntry[] {
	...
	for (const domain of domains) {
		const smoke = `e2e-device/specs/${domain}.smoke.spec.ts`;
		if (fs.existsSync(path.join(repoRoot(), smoke))) {
			cases.push({
		...
		const nav = `e2e-device/specs/${domain}.nav.spec.ts`;
		if (
			fs.existsSync(path.join(repoRoot(), nav)) &&
