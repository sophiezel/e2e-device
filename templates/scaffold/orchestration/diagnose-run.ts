...
export function diagnoseRun(): DiagnoseRunResult {
	...
	if (fs.existsSync(authPath)) {
		...
		recommendations.push(
			"读取 e2e-device/artifacts/auth-recovery.json，向用户询问 E2E_ACCOUNT/E2E_PASSWORD（仅写入 env），ensureLoggedIn 后使用 init.sh --sequential 重跑失败 case",
		);
	}
