...
export function checkL2Readiness(options?: {
	...
	if (fs.existsSync(registry)) {
		...
		const missing = registryCases.filter(
			(c) => !fs.existsSync(path.join(repoRoot(), c.spec)),
		);
		if (runL2 && missing.length > 0) {
			blockers.push({
				...
				resolution: "补全 e2e-device/specs 或运行 discover-cases --union",
			});
		}
	}
	...
	if (runL2 && !l1Only) {
		...
		for (const route of routes) {
			...
			const fp = path.join(repoRoot(), manifest.mock!.fixtureDir, route.fixture);
			if (!fs.existsSync(fp)) {
				blockers.push({
					id: "fixtures_missing",
					severity: "blocker",
